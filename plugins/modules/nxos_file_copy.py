...
def md5sum_check(module, dst, file_system):
    ...
    remote_filehash = run_commands(
        module, {"command": command, "output": "text"}
    )[0]
...
def copy_file_from_remote(
    module, local, local_file_directory, file_system="bootflash:"
):
    hostname = module.params["host"]
    username = module.params["username"]
    password = module.params["password"]
    ...
    try:
        child = pexpect.spawn(
            "ssh " + username + "@" + hostname + " -p" + str(port)
        )
        ...
        index = child.expect(
            ["timed out", "existing", "yes", "(?i)password"], timeout=180
        )
        if index == 0:
            module.fail_json(
                msg="Timeout occured due to remote scp server not responding"
            )
        elif index == 1:
            child.sendline("y")
            # response could be unknown host addition or Password
        ...
        if index == 0:
            module.fail_json(
                msg="File copy failed due to no space left on the device"
            )
        elif index == 1:
            module.fail_json(
                msg="Username/Password for remote scp server is wrong"
            )
        elif index == 2:
            module.fail_json(
                msg="File copy failed due to remote file not present"
            )
        elif index == 3:
            module.fail_json(
                msg='Timeout occured, please increase "file_pull_timeout" and try again!'
            )
    except pexpect.ExceptionPexpect as e:
        module.fail_json(
            msg="%s" % to_native(e), exception=traceback.format_exc()
        )
    ...
    if file_pull:
        ...
        if not module.check_mode:
            copy_file_from_remote(
                module, local, local_file_directory, file_system=file_system
            )
        ...
        remote_exists = remote_file_exists(
            module, dest, file_system=file_system
        )
